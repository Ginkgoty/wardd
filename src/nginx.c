...
)
{
    ...
    static const char server_contents[] =
        ...
        "if ($wardd_client_is_cn = 0) {\n"
        "    return 444;\n"
        "}\n";

    if (error != NULL && error_size > 0) error[0] = '\0';
    if (ensure_directory(generated_directory, error, error_size) != 0) return -1;
    if (snprintf(include_path, sizeof(include_path), "%s/current/nginx-cn.conf", generated_directory) >=
            (int)sizeof(include_path) ||
        snprintf(geo_path, sizeof(geo_path), "%s/wardd-geo.conf", generated_directory) >=
            (int)sizeof(geo_path) ||
        snprintf(server_path, sizeof(server_path), "%s/wardd-cn-only.conf", generated_directory) >=
...
int wardd_nginx_test_live(const char *nginx_binary, char *error, size_t error_size)
{
    ...
    return run_live_command(nginx_binary, arguments, "live configuration test", error, error_size);
}
...
int wardd_nginx_reload_live(const char *nginx_binary, char *error, size_t error_size)
{
    ...
    return run_live_command(nginx_binary, arguments, "reload", error, error_size);
}
...
)
{
    ...
    result = run_live_command(
        nginx_binary, arguments, "generated http integration test", error, error_size
    );
